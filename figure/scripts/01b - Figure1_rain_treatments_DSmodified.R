...
crdata<-fulldata%>%filter(site%in%"costarica")
...
panelA <- ggplot() +
  ...
  geom_point(data = crdata, mapping = aes(x = intended.mu, y = intended.k, fill = site),
             colour = "grey80", shape = 21, size = 3, alpha = 0.5, na.rm = TRUE) + 
  geom_point(data = sitemean, mapping = aes(x = mu_mean, y = k_mean, fill = site),
              colour = "black", shape = 21, size = 5, alpha = 1, na.rm = TRUE) +
...
panelB <- ggplot(data = site_treatment_data) +
  facet_wrap(~ site) + 
...
ggsave(filename = "figure/figures/Figure_1_parameter_space2.png", plot = panelA, width = 120, height = 89, dpi = 600, units = "mm")
